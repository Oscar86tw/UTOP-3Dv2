...
function makeModel(item){
 ...
 if(item.type==='barrier'){
   ...
   const thickness=.055;
   const frameHeight=.055;
   const frameMaterial=mat(p.detected?0x44dd77:0xe4b82f);

   const top=new THREE.Mesh(
     new THREE.BoxGeometry(width,frameHeight,thickness),
     frameMaterial
   );
   ...
   const left=new THREE.Mesh(
     new THREE.BoxGeometry(thickness,frameHeight,length),
     frameMaterial
   );
...
function renderPlan(){
 ...
 for(const i of state.items){
  ...
  if(i.type==='barrier'){
    ...
    frame.setAttribute('x',String(-width*pixelsPerMeter/2));
    frame.setAttribute('y',String(-length*pixelsPerMeter/2));
    frame.setAttribute('width',String(width*pixelsPerMeter));
    frame.setAttribute('height',String(length*pixelsPerMeter));
 ...
 for(const i of state.items){
  ...
  if(i&&i.type==='ledpanel'&&i.params&&i.params.state==='RUNNING'){
    ...
    if(i.runtime.lastDisplayedSecond!==displayedSecond){
      ...
      rebuild(i);
      renderPlan();
      if(i.id===state.selectedId)renderQuick(i);
    }
