...
function startPlanDrag(e){planDrag=get(e.currentTarget.dataset.id);select(planDrag.id);e.currentTarget.setPointerCapture(e.pointerId)}
...
$('planSvg').onpointerup=()=>planDrag=null;
...
function bindInspector(i){
 ...
 if($('timerSeconds'))$('timerSeconds').onchange=e=>{p.seconds=+e.target.value;p.remaining=p.seconds;rebuild(i);renderAll();markDirty()};
...
renderer.domElement.onpointerup=()=>{drag3d=null;orbit.enabled=true};
...
function renderWiring(){ensureWireNodePositions();normalizeWireColors();
 ...
 $('wireList').innerHTML=state.wires.map(w=>`<div class="wire-item"><i class="wire-color-dot" style="background:${w.color}"></i><span>${get(w.sourceId)?.name}.${w.sourcePort} → ${get(w.targetId)?.name}.${w.targetPort}</span><button data-wire="${w.id}">刪除</button></div>`).join('')||'<div>尚未接線</div>';
 document.querySelectorAll('[data-wire]').forEach(b=>b.onclick=()=>{state.wires=state.wires.filter(w=>w.id!==b.dataset.wire);renderWiring();markDirty()});
...
state.items.forEach(i=>group.remove(i.mesh));state.items=[];state.wires=d.wires||[];state.selectedId=null;for(const s of d.items||[])add(s.type,s);renderAll();$('statusText').textContent='專案讀取完成'};
...
function tick(now){
 for(const i of state.items){
  ...
  if(i.type==='timer'&&i.params.state==='RUNNING'){
    if(!i.runtime.lastTick)i.runtime.lastTick=now;const dt=(now-i.runtime.lastTick)/1000;i.runtime.lastTick=now;i.params.remaining=Math.max(0,i.params.remaining-dt);
    if(now-lastTimerRender>180){rebuild(i);lastTimerRender=now}
    if(i.params.remaining<=0){i.params.state='DONE';propagate(i)}
    if(i.id===state.selectedId)renderInspector();renderPlan();
  }
 }
...
function selectWireForAnalysis(wireId){
  ...
  if(wire){
    ...
    $('wireInfoDrawer').classList.add('open');
...
function clearWireAnalysis(){
  ...
  $('wireInfoDrawer')?.classList.remove('open');
}
$('closeWireInfo')?.addEventListener('click',clearWireAnalysis);
