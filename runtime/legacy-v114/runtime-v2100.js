...
function makeModel(item){
 ...
 const m=new THREE.Mesh(new THREE.TorusGeometry(Math.max(.45,p.width/2),.035,10,56),mat(p.detected?0x44dd77:0xe4b82f));m.scale.z=Math.max(.7,p.length/Math.max(.1,p.width));m.rotation.x=Math.PI/2;m.position.y=.035;g.add(m);
 }else if(item.type==='infrared'){
 const half=Math.max(1,Number(p.distance)||4)/2;[-half,half].forEach((x,k)=>{const post=new THREE.Mesh(new THREE.BoxGeometry(.18,1.45,.18),mat(0x404a50));post.position.set(x,.725,0);g.add(post);const head=new THREE.Mesh(new THREE.BoxGeometry(.28,.34,.22),mat(0x171b1e));head.position.set(x,1.32,0);g.add(head);const lens=new THREE.Mesh(new THREE.SphereGeometry(.055,14,8),mat(k?0x37c9ff:0xff4b4b));lens.position.set(x,1.33,.13);g.add(lens)});if(p.showBeam!==false){const beam=new THREE.Mesh(new THREE.CylinderGeometry(.018,.018,Math.max(2,Number(p.distance)||4),12),new THREE.MeshBasicMaterial({color:p.blocked?0xff3535:0x43dfff,transparent:true,opacity:p.blocked?.2:.72}));beam.rotation.z=Math.PI/2;beam.position.y=1.33;g.add(beam)}
...
function input(i,key){
 ...
 }else if(i.type==='ledpanel'){if(key==='red')p.mode='red';if(key==='green')p.mode='green';if(key==='off')p.mode='off';if(key==='start'){p.state='RUNNING';if(p.remaining<=0)p.remaining=p.seconds;i.runtime.lastTick=performance.now()}if(key==='reset'){p.state='IDLE';p.remaining=p.seconds;i.runtime.lastTick=0}rebuild(i);
...
function renderPlan(){
 ...
 for(const i of state.items){
  ...
  if(i.type==='barrier'){
    ...
    const half=Math.max(45,p.distance*12);[-half,half].forEach(x=>{const q=document.createElementNS('http://www.w3.org/2000/svg','rect');q.setAttribute('x',String(x-7));q.setAttribute('y','-20');q.setAttribute('width','14');q.setAttribute('height','40');q.setAttribute('fill','#3d474d');model.appendChild(q)});const line=document.createElementNS('http://www.w3.org/2000/svg','line');line.setAttribute('x1',String(-half));line.setAttribute('x2',String(half));line.setAttribute('y1','0');line.setAttribute('y2','0');line.setAttribute('stroke',p.blocked?'#ff3f3f':'#44dfff');line.setAttribute('stroke-width','4');line.setAttribute('stroke-dasharray','8 5');model.appendChild(line);
...
function bindInspector(i){
 ...
 if($('loopWidth'))$('loopWidth').onchange=e=>{p.width=Math.max(.8,Math.min(4,Number(e.target.value)||1.8));rebuild(i);renderAll();markDirty()};
 if($('loopLength'))$('loopLength').onchange=e=>{p.length=Math.max(1,Math.min(6,Number(e.target.value)||3));rebuild(i);renderAll();markDirty()};
 if($('irDistance'))$('irDistance').onchange=e=>{p.distance=Math.max(1,Math.min(20,Number(e.target.value)||4));rebuild(i);renderAll();markDirty()};
 if($('irShowBeam'))$('irShowBeam').onchange=e=>{p.showBeam=e.target.checked;rebuild(i);renderAll();markDirty()};
 ...
 if($('ledSeconds'))$('ledSeconds').onchange=e=>{p.seconds=Math.max(1,Math.min(99,Number(e.target.value)||10));p.remaining=p.seconds;rebuild(i);renderAll();markDirty()};
 if($('ledMode'))$('ledMode').onchange=e=>{p.mode=e.target.value;rebuild(i);renderAll();markDirty()};
...
function tick(now){
 for(const i of state.items){
  ...
  if(i.type==='ledpanel'&&i.params.state==='RUNNING'){
    if(!i.runtime.lastTick)i.runtime.lastTick=now;
    const dt=(now-i.runtime.lastTick)/1000;i.runtime.lastTick=now;
    i.params.remaining=Math.max(0,i.params.remaining-dt);
    if(i.params.remaining<=0){i.params.remaining=0;i.params.state='DONE';i.runtime.lastTick=0;rebuild(i);renderPlan();if(i.id===state.selectedId)renderInspector();propagate(i)}
    else if(now-lastTimerRender>180){rebuild(i);renderPlan();lastTimerRender=now}
  }
 }
 }
