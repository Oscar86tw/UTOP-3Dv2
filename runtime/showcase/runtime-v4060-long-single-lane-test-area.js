...
function add(type,saved={}){
 ...
 const item={
   ...
   x:saved.x??0,
   y:normalizeHeight(savedHeight,-20,20),
   z:saved.z??0,
   params:{
     ...clone(d.defaults),
     installationHeight:normalizeHeight(savedHeight,-20,20),
     partsVisibility:{
       bracket:true,
       base:true,
...
function rebuild(i){
 group.remove(i.mesh);
 i.mesh=makeModel(i);
 i.y=normalizeHeight(i.y??i.params?.installationHeight??0,-20,20);
 ...
 if(state.activeTab==='properties'){
   ...
   <label>Y 安裝高度
    <div class="height-editor">
      ...
      <input id="fY" type="number" min="-20" max="20" step=".1" value="${Number(i.y||0).toFixed(1)}">
      <button id="heightUp" type="button">＋</button>
    </div>
    <input id="heightRange" type="range" min="-20" max="20" step=".1" value="${Number(i.y||0)}">
   </label>
...
function renderWiring(){ensureWireNodePositions();normalizeWireColors();
 ...
 if(wiring3DState?.mode==='3d')setTimeout(renderWiring3D,0);
}
...
/* =========================================================
   V4.0.6 3D Wiring View
   ========================================================= */
const wiring3DState={
 mode:'2d',renderer:null,scene:null,camera:null,controls:null,
 root:null,animationId:0,resizeObserver:null,nodeObjects:new Map(),wireObjects:new Map()
};

function createWiringLabelSprite(text){
 const canvas=document.createElement('canvas');
 canvas.width=512;canvas.height=96;
 const context=canvas.getContext('2d');
 context.fillStyle='rgba(5,18,28,.92)';
 context.roundRect?.(4,4,504,88,16);context.fill();
 context.strokeStyle='#70c2ef';context.lineWidth=3;context.stroke();
 context.fillStyle='#ffffff';context.font='bold 28px sans-serif';
 context.textAlign='center';context.textBaseline='middle';
 const short=text.length>22?text.slice(0,21)+'…':text;
 context.fillText(short,256,48);
 const texture=new THREE.CanvasTexture(canvas);
 texture.colorSpace=THREE.SRGBColorSpace;
 const material=new THREE.SpriteMaterial({map:texture,transparent:true,depthTest:false});
 const sprite=new THREE.Sprite(material);
 sprite.scale.set(3.5,.66,1);
 sprite.userData.dispose=()=>{texture.dispose();material.dispose()};
 return sprite;
}

function disposeWiring3DObject(object){
 object.traverse(child=>{
  child.geometry?.dispose?.();
...
function clearWiring3DScene(){
 if(!wiring3DState.root)return;
 disposeWiring3DObject(wiring3DState.root);
 wiring3DState.scene.remove(wiring3DState.root);
 wiring3DState.root=null;
 wiring3DState.nodeObjects.clear();
 wiring3DState.wireObjects.clear();
}

function initWiring3D(){
 const mount=$('wiring3DMount');
 if(!mount||wiring3DState.renderer)return;
 const scene=new THREE.Scene();
 scene.background=new THREE.Color(document.documentElement.dataset.theme==='light'?0xc8d5dc:0x07121b);
 scene.fog=new THREE.Fog(scene.background,32,72);
 ...
 const hemi=new THREE.HemisphereLight(0xeaf7ff,0x26333b,1.35);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xffffff,1.45);sun.position.set(10,18,12);sun.castShadow=true;scene.add(sun);
 const grid=new THREE.GridHelper(55,44,0x4a7188,0x24404f);grid.position.y=-.04;scene.add(grid);
 const floor=new THREE.Mesh(new THREE.PlaneGeometry(55,55),new THREE.MeshStandardMaterial({color:0x0b1b26,roughness:.94,metalness:0}));
 floor.rotation.x=-Math.PI/2;floor.position.y=-.06;floor.receiveShadow=true;scene.add(floor);
 Object.assign(wiring3DState,{renderer,scene,camera,controls});
 const resize=()=>{
 ...
 wiring3DState.resizeObserver=new ResizeObserver(resize);
 wiring3DState.resizeObserver.observe(mount);resize();
 const animate=()=>{
  wiring3DState.animationId=requestAnimationFrame(animate);
 ...
 renderer.domElement.addEventListener('dblclick',event=>{
  ...
  const hits=raycaster.intersectObjects([...wiring3DState.nodeObjects.values()],true);
  const id=hits[0]?.object?.userData?.deviceId||hits[0]?.object?.parent?.userData?.deviceId;
  if(id){select(id);renderAll();}
 });
}

function wiringLayerForItem(item,index){
 const type=item.type||'';
 if(['controller','relay','delaytimer','offdelayrelay','ondelaytimer','signalcontroller2','signalcontroller3','powersupply','poeswitch'].includes(type))return 4.5;
 if(['loopdetector','radar','infrared','loop','uhf','cardreader','lpr','ipcamera'].includes(type))return 2.1;
 if(['barrier','shutter','traffic','ledpanel','laneindicator','beacon','bollard'].includes(type))return .45;
 return 3+(index%2)*.8;
}

function renderWiring3D(){
 if(wiring3DState.mode!=='3d')return;
 initWiring3D();
 clearWiring3DScene();
 const root=new THREE.Group();root.name='WIRING_3D_ROOT';wiring3DState.scene.add(root);wiring3DState.root=root;
 ensureWireNodePositions();normalizeWireColors();
 const positions=new Map();
 const count=Math.max(1,state.items.length);
 state.items.forEach((item,index)=>{
  const wp=item.wirePosition||{x:40+(index%4)*310,y:50+Math.floor(index/4)*220};
  const x=(wp.x-500)/85;
  const z=(wp.y-300)/75;
  const y=wiringLayerForItem(item,index);
  positions.set(item.id,new THREE.Vector3(x,y,z));
  const group3d=new THREE.Group();group3d.position.set(x,y,z);group3d.userData.deviceId=item.id;
  const selected=item.id===state.selectedId;
  const boxMat=new THREE.MeshStandardMaterial({color:selected?0x8f6816:0x17384b,roughness:.56,metalness:.18,emissive:selected?0x5a3b00:0x06141d,emissiveIntensity:selected?.75:.18});
  const box=new THREE.Mesh(new THREE.BoxGeometry(3.0,1.15,1.65),boxMat);box.castShadow=true;box.receiveShadow=true;box.userData.deviceId=item.id;group3d.add(box);
  const edge=new THREE.LineSegments(new THREE.EdgesGeometry(box.geometry),new THREE.LineBasicMaterial({color:selected?0xffd15b:0x70c2ef}));edge.userData.deviceId=item.id;group3d.add(edge);
  const defsItem=defs[item.type]||{do:[],di:[]};
  const maxPorts=Math.max(defsItem.do.length,defsItem.di.length,1);
  defsItem.do.forEach((port,pIndex)=>{
   const py=.36-(pIndex/(Math.max(maxPorts-1,1)))*.72;
   const sphere=new THREE.Mesh(new THREE.SphereGeometry(.095,14,10),new THREE.MeshStandardMaterial({color:0xf4a62a,emissive:0x8a4300,emissiveIntensity:.8}));
   sphere.position.set(-1.58,py,0);group3d.add(sphere);
  });
  defsItem.di.forEach((port,pIndex)=>{
   const py=.36-(pIndex/(Math.max(maxPorts-1,1)))*.72;
   const sphere=new THREE.Mesh(new THREE.SphereGeometry(.095,14,10),new THREE.MeshStandardMaterial({color:0x4eb5ef,emissive:0x084b75,emissiveIntensity:.8}));
   sphere.position.set(1.58,py,0);group3d.add(sphere);
  });
  const label=createWiringLabelSprite(item.name||item.type||'設備');label.position.set(0,1.05,0);group3d.add(label);
  root.add(group3d);wiring3DState.nodeObjects.set(item.id,group3d);
 });

 state.wires.forEach((wire,index)=>{
  const source=positions.get(wire.sourceId),target=positions.get(wire.targetId);
  if(!source||!target)return;
  const start=source.clone().add(new THREE.Vector3(-1.62,0,0));
  const end=target.clone().add(new THREE.Vector3(1.62,0,0));
  const lift=Math.max(start.y,end.y)+1.4+(index%5)*.62;
  const spread=(index%3-1)*.55;
  const points=[start,new THREE.Vector3(start.x-1.0,lift,start.z+spread),new THREE.Vector3(end.x+1.0,lift,end.z-spread),end];
  ...
  const geometry=new THREE.TubeGeometry(curve,48,selected?.105:.065,8,false);
  const material=new THREE.MeshStandardMaterial({color,emissive:color,emissiveIntensity:selected?1.4:.48,roughness:.35,metalness:.12,transparent:true,opacity:selected?1:.86});
  const tube=new THREE.Mesh(geometry,material);tube.userData.wireId=wire.id;root.add(tube);wiring3DState.wireObjects.set(wire.id,tube);
  const startBall=new THREE.Mesh(new THREE.SphereGeometry(.12,14,10),new THREE.MeshStandardMaterial({color:0xf4a62a,emissive:0xf4a62a,emissiveIntensity:.9}));startBall.position.copy(start);root.add(startBall);
  const endBall=new THREE.Mesh(new THREE.SphereGeometry(.12,14,10),new THREE.MeshStandardMaterial({color:0x4eb5ef,emissive:0x4eb5ef,emissiveIntensity:.9}));endBall.position.copy(end);root.add(endBall);
 });
 const box=new THREE.Box3().setFromObject(root);
 if(!box.isEmpty()){
  const center=box.getCenter(new THREE.Vector3());
  wiring3DState.controls.target.copy(center);
 }
}

function resetWiring3DView(){
 initWiring3D();
 wiring3DState.camera.position.set(15,18,22);
 wiring3DState.controls.target.set(0,2,0);
 wiring3DState.controls.update();
}
...
function setWiringView(mode){
 ...
 if(is3D){requestAnimationFrame(()=>{renderWiring3D();wiring3DState.resizeObserver?.disconnect();const mount=$('wiring3DMount');if(mount){wiring3DState.resizeObserver?.observe(mount);}})}
 else{requestAnimationFrame(drawWires)}
 window.UTOP_DEBUG?.record?.('ACTION','Wiring',`切換為${is3D?'3D':'2D'}接線視圖`);
}
...
window.addEventListener('utop-theme-changed',()=>{
 if(wiring3DState.scene){
  wiring3DState.scene.background=new THREE.Color(document.documentElement.dataset.theme==='light'?0xc8d5dc:0x07121b);
  wiring3DState.scene.fog.color.copy(wiring3DState.scene.background);
 }
});
window.UTOP_WIRING_VIEW=Object.freeze({setMode:setWiringView,refresh3D:renderWiring3D,reset3D:resetWiring3DView});

$('wiringBtn').onclick=()=>{$('wiringDialog').showModal();renderWiring();setTimeout(()=>{if(wiring3DState.mode==='3d')renderWiring3D()},0)};$('closeWiring').onclick=()=>$('wiringDialog').close();$('cancelWire').onclick=()=>{state.pending=null;$('wireHint').textContent='尚未選擇來源';renderWiring()};$('clearWires').onclick=()=>{if(confirm('清除全部接線？')){state.wires=[];renderWiring();renderAll();markDirty()}};
...
function selectWireForAnalysis(wireId){
  ...
  if(wiring3DState.mode==='3d')renderWiring3D();
}
...
/* V4.0.6 Collapsible Sidebar Inspector */
(function(){
...
/* V4.0.6 Unlimited Loop Dimensions */
(function(){
...
/* V4.0.6 Realistic Rendering Foundation */
(async function(){
...
/* =========================================================
   V4.0.6 Reliable Scene Brightness
   ========================================================= */
(function(){
...
/* =========================================================
   V4.0.6 Mobile Workspace Helpers
   ========================================================= */
...
/* =========================================================
   V4.0.6 Professional 2D Selection Engine
   ========================================================= */
(function(){
...
/* V4.0.6: map unlabeled SVG groups to the nearest device label/item. */
(function(){
...
/* =========================================================
   V4.0.6 Stable Scene Plugin Host API
   ========================================================= */
...
/* =========================================================
   V4.0.6 Monte Carlo Scene UI Bridge
   ========================================================= */
(function(){
