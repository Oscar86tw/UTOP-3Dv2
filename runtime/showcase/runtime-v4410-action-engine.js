...
function environmentFxAnimate(){
 ...
 if(
  ...
  ['typhoon','storm','tornado','tsunami'].includes(environmentFxState.type)
 ){
  if(!environmentFxState.baseCameraPosition){
   environmentFxState.baseCameraPosition=camera.position.clone();
  }
...
function renderPlanSceneGeometry(){
 ...
 const bounds=getSafeSceneWorldBounds();

 const sceneWidth=Math.max(1,bounds.maxX-bounds.minX);
 const sceneDepth=Math.max(1,bounds.maxZ-bounds.minZ);
 ...
 // 依場景類型同步主要結構。
 if(/snow/i.test(sceneId)||/雪/.test(sceneId)){
  appendPlanSceneRect(layer,{
   x:centerX,
   z:centerZ,
   width:sceneWidth*.92,
 ...
 const roadWidth=Math.max(5,sceneWidth*.46);
 appendPlanSceneRect(layer,{
  x:centerX,
  z:centerZ,
  width:roadWidth,
 ...
 const sidewalkWidth=Math.max(1.5,sceneWidth*.13);
 const sidewalkOffset=(roadWidth+sidewalkWidth)/2;

 appendPlanSceneRect(layer,{
  x:centerX-sidewalkOffset,
  z:centerZ,
  width:sidewalkWidth,
  depth:sceneDepth*.88,
  fill:'#d8d6cc',
 ...
 const grassWidth=Math.max(1,sceneWidth*.08);
 appendPlanSceneRect(layer,{
 ...
 // 中央虛線。
 const ppm=getPlanPixelsPerMeter();
 const center=worldToPlan(centerX,centerZ);
 ...
 line.setAttribute(
  'y1',
  String(center.y-sceneDepth*.40*ppm.z)
 );
 ...
 // 方向箭頭。
 [-sceneDepth*.22,sceneDepth*.22].forEach((offset,index)=>{
  const p=worldToPlan(centerX,centerZ+offset);
...
function rebuildPlanModelAsTrueTopView(model,item,planRotation){
 // V4.4.1.1.1：停用通用矩形／橢圓重畫，恢復V4.0.2原圖。
 return null;
 /*
...
function renderPlan(){
 ...
 for(const i of state.items){
  ...
  // V4.4.1.1.1：恢復 V4.0.2 原始專屬2D模組圖案。
  // 圖案外觀不再由通用矩形／圓形覆蓋。
if(i.type==='barrier'){
  ...
  // V4.4.1.1：完成每個2D設備後，固定掛入plan root。
...
function updatePlanDrag(e){
 ...
 safeRefreshItemVisual(planDrag);
...
function safeRefreshItemVisual(item){
 try{
  if(typeof syncItemTransform==='function'){
   syncItemTransform(item);
   return {ok:true,method:'syncItemTransform'};
  }
  if(typeof updateItemTransform==='function'){
   updateItemTransform(item);
   return {ok:true,method:'updateItemTransform'};
  }
  if(typeof refreshItem==='function'){
   refreshItem(item);
   return {ok:true,method:'refreshItem'};
  }

  if(item?.mesh){
   if(Number.isFinite(Number(item.x)))item.mesh.position.x=Number(item.x);
   if(Number.isFinite(Number(item.y)))item.mesh.position.y=Number(item.y);
   if(Number.isFinite(Number(item.z)))item.mesh.position.z=Number(item.z);
  }

  if(typeof renderPlan==='function')renderPlan();
  if(typeof renderInspector==='function')renderInspector();
  return {ok:true,method:'fallbackRender'};
 }catch(error){
  window.UTOP_DEBUG?.record?.(
   'ERROR','ActionEngine',
   `設備畫面更新失敗：${item?.name||item?.type||'unknown'}`,
   {id:item?.id,type:item?.type,message:error?.message,stack:error?.stack}
  );
...
function executeDeviceAction(item,action,payload={}){
 ...
 if(type==='barrier'){
  if(action==='open'){item.params.angle=90;item.params.state='open';changed=true;}
  if(action==='close'){item.params.angle=0;item.params.state='closed';changed=true;}
  if(action==='stop'){item.params.state='stopped';changed=true;}
 }else if(type==='traffic'&&['red','green','off'].includes(action)){
  item.params.mode=action;changed=true;
 }else if(type==='relay'){
  if(action==='on'||action==='no'){item.params.state='NO';item.params.active=true;changed=true;}
  if(action==='off'||action==='nc'){item.params.state='NC';item.params.active=false;changed=true;}
 }else if(['timer','delaytimer','powerondelay','poweroffdelay'].includes(type)){
  if(action==='start'){
   item.params.running=true;
   item.params.remaining=Number(payload.seconds??item.params.delay??item.params.seconds??0);
   changed=true;
  }
  if(action==='stop'||action==='reset'){
   item.params.running=false;item.params.remaining=0;changed=true;
  }
 }

 if(!changed){
  const result={ok:false,id:item.id,type:item.type,action,reason:'unsupported-action'};
  window.UTOP_DEBUG?.record?.('WARNING','ActionEngine','設備不支援指定動作',result);
  return result;
 }

 const refresh=safeRefreshItemVisual(item);
 if(typeof renderPlan==='function')renderPlan();
 if(typeof renderInspector==='function')renderInspector();
 if(typeof markDirty==='function')markDirty();

 const result={ok:true,id:item.id,type:item.type,action,refresh};
 window.UTOP_DEBUG?.record?.(
  'ACTION','ActionEngine',
  `設備動作：${item.name||item.type} → ${action}`,
  result
 );
 return result;
}
...
const REHEARSAL_SCENARIOS=Object.freeze({
 singleLaneEntry:{
  id:'singleLaneEntry',
  name:'單車道進入',
  steps:[
   {
    id:'vehicle-approach',
    title:'車輛進入感應範圍',
    deviceTypes:['uhf','radar'],
    deviceLabel:'ETAG／雷達',
    wait:1,
    next:'ETAG讀頭',
    signal:'vehicleDetected'
   },
   {
    id:'etag-read',
    title:'ETAG讀取車號資料',
   ...
   },
   {
    id:'timer-delay',
    title:'延遲計時器開始倒數',
    deviceTypes:['timer','delaytimer'],
    deviceLabel:'延遲計時器',
    wait:2,
    next:'柵欄機',
    signal:'delayComplete'
   },
   {
    id:'barrier-close',
    title:'柵欄機安全下降',
...
function getRehearsalScenario(){
 return REHEARSAL_SCENARIOS[rehearsalState.scenarioId];
}

function getRehearsalItemsForStep(step){
 if(!step)return [];
 const resolved=resolveSceneItemsByTypes(step.deviceTypes||[]);
 if(!resolved.length){
  window.UTOP_DEBUG?.record?.(
   'WARNING','Rehearsal',
   `步驟找不到場景設備：${step.title}`,
   {
    stepId:step.id,
    requestedTypes:step.deviceTypes,
    availableDevices:state.items.map(item=>({
...
function clearRehearsalHighlight(){
 state.items.forEach(item=>{
  item.mesh?.traverse?.(node=>{
   if(node?.material&&node.userData?.rehearsalOriginalEmissive!==undefined){
    if(node.material.emissive){
     node.material.emissive.setHex(
      node.userData.rehearsalOriginalEmissive
     );
    }
    if(node.userData.rehearsalOriginalEmissiveIntensity!==undefined){
     node.material.emissiveIntensity=
      node.userData.rehearsalOriginalEmissiveIntensity;
    }
    delete node.userData.rehearsalOriginalEmissive;
    delete node.userData.rehearsalOriginalEmissiveIntensity;
   }
  });
 });
 document.querySelectorAll('.rehearsal-device-active')
  .forEach(node=>node.classList.remove('rehearsal-device-active'));
}

function highlightRehearsalItems(items){
 ...
 items.forEach(item=>{
  item.mesh?.traverse?.(node=>{
   ...
   if(node.material.emissive){
    node.userData.rehearsalOriginalEmissive=
     node.material.emissive.getHex();
    node.userData.rehearsalOriginalEmissiveIntensity=
     Number(node.material.emissiveIntensity)||0;
    node.material.emissive.setHex(0x24bfff);
    node.material.emissiveIntensity=.9;
   }
  });
  ...
  document
   ...
   .forEach(node=>node.classList.add('rehearsal-device-active'));
 });
}

function emitRehearsalSignal(step,items){
 const detail={
  scenarioId:rehearsalState.scenarioId,
  stepIndex:rehearsalState.currentIndex,
  step,
  itemIds:items.map(item=>item.id),
  signal:step?.signal
 };
 window.dispatchEvent(new CustomEvent('utop:rehearsal-signal',{detail}));
 window.UTOP_WIRING_ENGINE?.emitSignal?.(detail);
 window.UTOP_SIMULATION_ENGINE?.handleRehearsalStep?.(detail);

 let results=[];
 if(step?.signal==='relayOn')results=executeDeviceActionForItems(items,'on');
 if(step?.signal==='barrierOpen')results=executeDeviceActionForItems(items,'open');
 if(step?.signal==='barrierClose')results=executeDeviceActionForItems(items,'close');
 if(step?.signal==='systemReady')results=executeDeviceActionForItems(items,'red');
 if(step?.signal==='delayComplete')results=executeDeviceActionForItems(items,'stop');

 window.UTOP_DEBUG?.record?.(
  results.some(result=>result.ok===false)?'WARNING':'INFO',
  'ActionEngine',
  `演練訊號處理：${step?.signal||'none'}`,
  {
   signal:step?.signal,
   matchedItems:items.map(item=>({id:item.id,type:item.type,name:item.name})),
   results
  }
 );
 if(typeof renderPlan==='function')renderPlan();
}
function renderRehearsalConsole(){
 const scenario=getRehearsalScenario();
 const total=scenario.steps.length;
 const index=rehearsalState.currentIndex;
 const step=index>=0?scenario.steps[index]:null;
 const progress=index<0?0:((index+1)/total)*100;

 if($('rehearsalScenarioName')){
  $('rehearsalScenarioName').textContent=scenario.name;
 }
 if($('rehearsalProgressBar')){
  $('rehearsalProgressBar').style.width=`${progress}%`;
 }
 if($('rehearsalStepCounter')){
  $('rehearsalStepCounter').textContent=
   `步驟 ${Math.max(0,index+1)} / ${total}`;
 }
 if($('rehearsalStateLabel')){
  const labels={
   idle:'尚未開始',
   running:'演練中',
   paused:'已暫停',
  ...
  $('rehearsalStateLabel').textContent=
   labels[rehearsalState.status]||rehearsalState.status;
 }
 if($('rehearsalCurrentAction')){
  $('rehearsalCurrentAction').textContent=
   step?.title||'等待開始';
 }
 if($('rehearsalCurrentDevice')){
  $('rehearsalCurrentDevice').textContent=
   step?.deviceLabel||'—';
 }
 if($('rehearsalWaitTime')){
  const wait=
   rehearsalState.status==='paused'
    ?rehearsalState.remaining
    :(step?.wait||0);
  $('rehearsalWaitTime').textContent=
   `${Math.max(0,Number(wait)||0).toFixed(1)} 秒`;
 }
 if($('rehearsalNextDevice')){
  $('rehearsalNextDevice').textContent=
   step?.next||'—';
 }

 const list=$('rehearsalStepList');
 if(list){
  list.innerHTML=scenario.steps.map((item,itemIndex)=>{
   const cls=[
    'rehearsal-step-chip',
    itemIndex===index?'is-current':'',
    itemIndex<index?'is-complete':''
   ].filter(Boolean).join(' ');
 ...
 $('rehearsalEngineeringMode')?.classList.toggle(
  'is-active',
  rehearsalState.mode==='engineering'
 );
 $('rehearsalPresentationMode')?.classList.toggle(
  'is-active',
  rehearsalState.mode==='presentation'
 );
...
function cancelRehearsalTimer(){
 if(rehearsalState.timer){
  clearTimeout(rehearsalState.timer);
  rehearsalState.timer=null;
 }
}

function scheduleRehearsalAdvance(waitSeconds){
 cancelRehearsalTimer();

 const wait=Math.max(0,Number(waitSeconds)||0);
 rehearsalState.remaining=wait;
 rehearsalState.dueAt=performance.now()+wait*1000;
 const token=rehearsalState.runToken;

 rehearsalState.timer=setTimeout(()=>{
  if(
   token!==rehearsalState.runToken||
   rehearsalState.status!=='running'
  )return;
  advanceRehearsalStep(1,true);
 },wait*1000);
}

function applyRehearsalStep(index,{auto=false}={}){
 const scenario=getRehearsalScenario();
 const total=scenario.steps.length;

 if(index<0){
  rehearsalState.currentIndex=-1;
  rehearsalState.status='idle';
  clearRehearsalHighlight();
  renderRehearsalConsole();
  return;
 }

 if(index>=total){
  rehearsalState.currentIndex=total-1;
  rehearsalState.status='completed';
  cancelRehearsalTimer();
  clearRehearsalHighlight();
  renderRehearsalConsole();
  window.UTOP_DEBUG?.record?.(
   ...
   `演練完成：${scenario.name}`,
   {
    ...
    transitions:rehearsalState.transitionCount
   }
  );
  return;
 }

 rehearsalState.transitionCount++;
 if(rehearsalState.transitionCount>rehearsalState.maxTransitions){
  stopRehearsal('偵測到可能的無限迴圈，演練已停止');
  window.UTOP_DEBUG?.record?.(
   'ERROR',
   'Rehearsal',
   '演練超過安全步數，已阻止無限迴圈',
   {
    maxTransitions:rehearsalState.maxTransitions
   }
  );
  return;
 }

 rehearsalState.currentIndex=index;
 const step=scenario.steps[index];
 const items=getRehearsalItemsForStep(step);

 highlightRehearsalItems(items);
 emitRehearsalSignal(step,items);
 renderRehearsalConsole();
 ...
 if(rehearsalState.status==='running'){
  scheduleRehearsalAdvance(step.wait);
 }
}

function advanceRehearsalStep(delta=1,auto=false){
 cancelRehearsalTimer();
 applyRehearsalStep(
  rehearsalState.currentIndex+delta,
  {auto}
 );
}
...
function startRehearsal(){
 const scenario=getRehearsalScenario();
 rehearsalState.runToken++;
 rehearsalState.transitionCount=0;
 rehearsalState.status='running';

 if(
  rehearsalState.currentIndex<0||
  rehearsalState.currentIndex>=scenario.steps.length-1
 ){
  rehearsalState.currentIndex=-1;
 }
 advanceRehearsalStep(1,false);
}

function pauseRehearsal(){
 if(rehearsalState.status!=='running')return;
 rehearsalState.remaining=Math.max(
  0,
  (rehearsalState.dueAt-performance.now())/1000
 );
 rehearsalState.status='paused';
 cancelRehearsalTimer();
 renderRehearsalConsole();
...
function resumeRehearsal(){
 if(rehearsalState.status!=='paused')return;
 rehearsalState.status='running';
 renderRehearsalConsole();
 scheduleRehearsalAdvance(rehearsalState.remaining);
}

function stopRehearsal(message='演練已停止'){
 rehearsalState.runToken++;
 rehearsalState.status='stopped';
 cancelRehearsalTimer();
 clearRehearsalHighlight();
 renderRehearsalConsole();
...
function resetRehearsal(){
 ...
 rehearsalState.currentIndex=-1;
 rehearsalState.transitionCount=0;
 rehearsalState.remaining=0;
 cancelRehearsalTimer();
 clearRehearsalHighlight();

 state.items.forEach(item=>{
  const type=normalizeDeviceSearchText(item.type);
  if(type==='barrier')executeDeviceAction(item,'close');
  if(type==='traffic')executeDeviceAction(item,'red');
  if(type==='relay')executeDeviceAction(item,'off');
  if(['timer','delaytimer','powerondelay','poweroffdelay'].includes(type)){
   executeDeviceAction(item,'reset');
  }
 });
 renderPlan?.();
 renderRehearsalConsole();
...
function setRehearsalMode(mode){
 rehearsalState.mode=
  mode==='presentation'?'presentation':'engineering';
 renderRehearsalConsole();

 window.UTOP_DEBUG?.record?.(
  'ACTION',
  'Rehearsal',
  `切換演練模式：${rehearsalState.mode}`,
  {mode:rehearsalState.mode}
 );
}

function installRehearsalConsole(){
 if($('rehearsalConsole')?.dataset.installed==='true')return;
 if($('rehearsalConsole')){
  $('rehearsalConsole').dataset.installed='true';
 }

 $('rehearsalStart')?.addEventListener('click',()=>{
  if(rehearsalState.status==='paused')resumeRehearsal();
  else startRehearsal();
 });
 $('rehearsalPause')?.addEventListener('click',pauseRehearsal);
 $('rehearsalPrevious')?.addEventListener('click',()=>{
  rehearsalState.status='paused';
  advanceRehearsalStep(-1,false);
 });
 ...
 $('rehearsalStop')?.addEventListener('click',()=>stopRehearsal());
 $('rehearsalReset')?.addEventListener('click',resetRehearsal);
 $('rehearsalEngineeringMode')?.addEventListener(
  'click',
  ()=>setRehearsalMode('engineering')
 );
 $('rehearsalPresentationMode')?.addEventListener(
  'click',
  ()=>setRehearsalMode('presentation')
 );
 $('rehearsalCollapse')?.addEventListener('click',()=>{
  const console=$('rehearsalConsole');
  console?.classList.toggle('is-collapsed');
  if($('rehearsalCollapse')){
   $('rehearsalCollapse').textContent=
    console?.classList.contains('is-collapsed')?'＋':'－';
  }
 });
...
/* V4.4.1.1.1 Collapsible Sidebar Inspector */
(function(){
...
/* V4.4.1.1.1 Unlimited Loop Dimensions */
(function(){
...
/* V4.4.1.1.1 Realistic Rendering Foundation */
(async function(){
...
/* =========================================================
   V4.4.1.1.1 Reliable Scene Brightness
   ========================================================= */
(function(){
...
/* =========================================================
   V4.4.1.1.1 Mobile Workspace Helpers
   ========================================================= */
...
/* =========================================================
   V4.4.1.1.1 Professional 2D Selection Engine
   ========================================================= */
(function(){
...
/* V4.4.1.1.1: map unlabeled SVG groups to the nearest device label/item. */
(function(){
...
/* =========================================================
   V4.4.1.1.1 Stable Scene Plugin Host API
   ========================================================= */
...
/* =========================================================
   V4.4.1.1.1 Monte Carlo Scene UI Bridge
   ========================================================= */
(function(){
...
window.UTOP_REHEARSAL_ENGINE={
 getState:()=>({...rehearsalState}),
 getScenario:getRehearsalScenario,
 start:startRehearsal,
 pause:pauseRehearsal,
 resume:resumeRehearsal,
 stop:stopRehearsal,
 reset:resetRehearsal,
 next:()=>advanceRehearsalStep(1,false),
 previous:()=>advanceRehearsalStep(-1,false),
 setMode:setRehearsalMode
};
