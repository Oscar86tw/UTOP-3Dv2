...
function makeModel(item){
 ...
 if(item.type==='barrier'){
   ...
   if(shouldShowStand(p)){
    ...
    pole.position.y=.7*barrierHeightScale75;addStandPart(g,pole);
   ...
   const body=new THREE.Mesh(new THREE.BoxGeometry(.62,.72,.22),new THREE.MeshPhysicalMaterial({color:0xdfe3df,roughness:.3,clearcoat:.5}));body.position.y=.7*barrierHeightScale5*barrierHeightScale;g.add(body);
...
function getModelTopFootprint(item){
 const fallback={
  width:Math.max(.2,Number(item?.params?.width)||.8),
  depth:Math.max(.2,Number(item?.params?.length)||Number(item?.params?.depth)||.8)
 };
 ...
 try{
  ...
  return {
   width:Math.max(.08,size.x),
   depth:Math.max(.08,size.z)
  };
 }catch(error){
  console.warn('[UTOP-3D] 取得3D模型平面尺寸失敗',error);
  return fallback;
 }finally{
...
function syncPlanModelScale(model,item,planRotation){
 ...
 const desiredWidth=Math.max(8,footprint.width*ppm.x);
 const desiredHeight=Math.max(8,footprint.depth*ppm.z);
...
function renderPlan(){
 ...
 for(const i of state.items){
  ...
  if(i.type==='barrier'){
    ...
    const fullLength=Math.max(8,(Number(p.armLength)||4)*getPlanPixelsPerMeter().average);
  ...
  const syncedPlanModel=syncPlanModelScale(model,i,planRotation);

  // 透明點選區維持容易操作，但不改變設備圖形比例。
  if(syncedPlanModel){
...
/* V4.3.1 Collapsible Sidebar Inspector */
(function(){
...
/* V4.3.1 Unlimited Loop Dimensions */
(function(){
...
/* V4.3.1 Realistic Rendering Foundation */
(async function(){
...
/* =========================================================
   V4.3.1 Reliable Scene Brightness
   ========================================================= */
(function(){
...
/* =========================================================
   V4.3.1 Mobile Workspace Helpers
   ========================================================= */
...
/* =========================================================
   V4.3.1 Professional 2D Selection Engine
   ========================================================= */
(function(){
...
/* V4.3.1: map unlabeled SVG groups to the nearest device label/item. */
(function(){
...
/* =========================================================
   V4.3.1 Stable Scene Plugin Host API
   ========================================================= */
...
/* =========================================================
   V4.3.1 Monte Carlo Scene UI Bridge
   ========================================================= */
(function(){
