...
function makeModel(item){
 ...
 if(item.type==='barrier'){
   ...
   [2.08,1.65].forEach((y,index)=>{
    const hood=new THREE.Mesh(new THREE.CylinderGeometry(.21,.21,.23,28,1,false,0,Math.PI),visor);
    hood.rotation.x=Math.PI/2;hood.rotation.z=Math.PI;hood.position.set(0,y,.22);g.add(hood);
    const active=index===0?p.mode==='red':p.mode==='green';
    const colour=index===0?0xff1b1b:0x14dd66;
    const inactive=index===0?0x4a1010:0x0f3c1e;
    const lens=new THREE.Mesh(new THREE.SphereGeometry(.145,28,16),new THREE.MeshPhysicalMaterial({color:active?colour:inactive,roughness:.18,metalness:0,clearcoat:.7,clearcoatRoughness:.1,emissive:active?colour:inactive,emissiveIntensity:active?2.8:.08}));
    lens.position.set(0,y,.185);g.add(lens);
   });
   ...
   const doorHeight=p.state==='OPEN'?0.25:p.state==='CLOSED'?2.45:1.3;
...
function controlRollingShutter(item,key){
 ...
 if(key==='open'){
  ...
  p.state='OPENING';
  setTimeout(()=>{
   if(!state.items.includes(item)||item.params.state!=='OPENING')return;
   p.state='OPEN';
   rebuild(item);renderAll();renderQuick(item);renderInspector();
  ...
  if(p.safetyActive){
   ...
   p.state='CLOSING';
   setTimeout(()=>{
    if(!state.items.includes(item)||item.params.state!=='CLOSING')return;
    p.state='CLOSED';
    rebuild(item);renderAll();renderQuick(item);renderInspector();
   },350);
  }
 }else if(key==='safety'||key==='photoBeam'){
  p.safetyActive=true;
  if(p.state==='CLOSING'){
   p.state='OPENING';
   setTimeout(()=>{
 ...
 }else if(key==='clearSafety'){
  p.safetyActive=false;
 }else if(key==='autoClose'){
  p.autoCloseEnabled=!p.autoCloseEnabled;
  if(p.autoCloseEnabled&&p.state==='OPEN')scheduleShutterAutoClose(item);
 }else if(key==='reset'){
  p.fault=false;
  p.safetyActive=false;
  p.state='CLOSED';
   ...
   if(i.type==='barrier'){
    ...
    box.innerHTML=`<div class="form-grid">
      <section class="field-group"><h3>鐵捲門控制參數</h3>
       ...
       <label>關門等待秒數<input id="shutterCloseDelay" type="number" min="0" max="999" step="1" value="${p.closeDelaySeconds}"></label>
...
function statusText(i){const p=i.params;if(i.type==='barrier')return`狀態：${p.state}<br>桿子角度：${p.angle.toFixed(1)}°`;if(i.type==='shutter')return`狀態：${p.state}<br>自動關門：${p.autoCloseEnabled?'啟用':'關閉'}（${p.closeDelaySeconds}秒）<br>防壓：${p.safetyActive?'觸發':'正常'}`;if(i.type==='traffic')return`目前燈號：${p.mode}`;if(i.type==='timer')return`剩餘：${Math.ceil(p.remaining)} 秒<br>狀態：${p.state}`;if(i.type==='loop')return`車輛偵測：${p.detected?'有車':'無車'}`;if(i.type==='infrared')return`對射：${p.blocked?'遮斷':'正常'}｜${p.distance}m`;if(i.type==='uhf')return`讀取：${p.detected?'成功':'待機'}<br>${p.lastTag}`;if(i.type==='cardreader')return`卡機：${p.state}`;if(i.type==='ledpanel')return`燈號：${p.mode}<br>剩餘：${Math.ceil(p.remaining)}秒`;if(i.type==='lpr')return`辨識：${p.state}<br>${p.plate}`;return`繼電器：${p.on?'ON':'OFF'}`}
...
function renderQuick(i=get(state.selectedId)){
 ...
 else if(i.type==='shutter')$('quickContent').innerHTML=`<div class="quick-buttons"><button data-q="open">開門</button><button data-q="stop">停止</button><button data-q="close">關門</button><button data-q="safety">防壓</button><button data-q="reset">復歸</button></div><div class="status-box">${statusText(i)}</div>`;
...
/* V3.1.1 Collapsible Sidebar Inspector */
(function(){
...
/* V3.1.1 Unlimited Loop Dimensions */
(function(){
...
/* V3.1.1 Realistic Rendering Foundation */
(async function(){
...
/* =========================================================
   V3.1.1 Reliable Scene Brightness
   ========================================================= */
(function(){
...
/* =========================================================
   V3.1.1 Mobile Workspace Helpers
   ========================================================= */
...
/* =========================================================
   V3.1.1 Professional 2D Selection Engine
   ========================================================= */
(function(){
...
/* V3.1.1: map unlabeled SVG groups to the nearest device label/item. */
(function(){
