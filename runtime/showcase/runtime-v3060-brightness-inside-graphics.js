...
function makeModel(item){
 ...
 if(item.type==='barrier'){
   ...
   if(shouldShowStand(p)){
    const pole=new THREE.Mesh(
      new THREE.CylinderGeometry(.05,.05,1.5,16),
      mat(0x676767)
    );
    pole.position.y=.75;
    addStandPart(g,pole);
   ...
   const width=(Number.isFinite(Number(p.width))&&Number(p.width)>0)
     ? Number(p.width)
     : 1.8;
   const length=(Number.isFinite(Number(p.length))&&Number(p.length)>0)
     ? Number(p.length)
     : 3;
   const thickness=.06;
   ...
   [-half,half].forEach((x,index)=>{
     if(shouldShowStand(p)){
      const post=new THREE.Mesh(
        new THREE.BoxGeometry(.18,1.45,.18),
        mat(0x404a50,{roughness:.5,metalness:.28})
      );
 ...
 if(shouldShowStand(p)){
  const stand=new THREE.Mesh(
    ...
    mat(0x636b70,{roughness:.5,metalness:.35})
  );
  stand.position.y=.35;
  addStandPart(g,stand);
 }
 const body=new THREE.Mesh(new THREE.BoxGeometry(.9,1.1,.18),mat(0xe8ebed));body.position.y=1.18;g.add(body);const face=new THREE.Mesh(new THREE.BoxGeometry(.72,.82,.035),mat(p.detected?0x49b8ff:0x7d858a));face.position.set(0,.67,.105);g.add(face);
...
function renderPlan(){
 ...
 for(const i of state.items){
  ...
  if(i.type==='barrier'){
    ...
    // Use the same X-axis metre scale as every other plan object.
    // 3D separates the two heads by exactly `distance` metres,
    // so the 2D top view must use distance × PLAN_X_PPM.
    const half=distance*PLAN_X_PPM/2;
    const postWidth=Math.max(8,.18*PLAN_X_PPM);
    const postDepth=Math.max(12,.28*PLAN_Z_PPM);

    [-half,half].forEach((x,index)=>{
      const post=document.createElementNS('http://www.w3.org/2000/svg','rect');
      post.setAttribute('x',String(x-postWidth/2));
      post.setAttribute('y',String(-postDepth/2));
      post.setAttribute('width',String(postWidth));
      post.setAttribute('height',String(postDepth));
      post.setAttribute('rx','3');
      post.setAttribute('fill','#3d474d');
      post.setAttribute('stroke',index?'#37c9ff':'#ff4b4b');
    ...
    line.setAttribute('class','infrared-plan-beam');
...
/* V3.0.6 Collapsible Sidebar Inspector */
(function(){
...
/* V3.0.6 Unlimited Loop Dimensions */
(function(){
...
/* V3.0.6 Realistic Rendering Foundation */
(async function(){
...
/* =========================================================
   V3.0.6 Reliable Scene Brightness
   ========================================================= */
(function(){
