...
$('wiringBtn').onclick=()=>{$('wiringDialog').showModal();setWireDrawerOpen(false);renderWiring()};$('closeWiring').onclick=()=>$('wiringDialog').close();$('cancelWire').onclick=()=>{state.pending=null;$('wireHint').textContent='尚未選擇來源';renderWiring()};$('clearWires').onclick=()=>{if(confirm('清除全部接線？')){state.wires=[];renderWiring();renderAll();markDirty()}};
