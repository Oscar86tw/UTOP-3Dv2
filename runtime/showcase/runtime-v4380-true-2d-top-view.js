...
function syncPlanModelScale(model,item,planRotation){
 ...
 model.setAttribute(
  'transform',
  `rotate(${planRotation}) scale(${uniformScale})`
 );
...
function getPlanViewportGroup(){
 return $('planViewport')||$('planItems')?.parentNode;
}
...
function installPlanZoomControls(){
 ...
 svg.addEventListener('pointerdown',event=>{
  ...
  if((event.button===1||event.button===2||background)&&!event.target?.closest?.('.plan-item')){
   event.preventDefault();
...
function renderPlan(){
 ...
 for(const i of state.items){
  ...
  if(i.type==='barrier'){
    const metrics=getBarrierPlanMetrics(i);
    const sign=p.armSide==='right'?1:-1;
    const bodyWidth=metrics.bodyWidthPx;
    const bodyDepth=metrics.bodyDepthPx;
    const armLength=metrics.armLengthPx;
    const pivotX=sign*bodyWidth*.34;
    const armStartX=sign*(bodyWidth*.5);

    // 2D直接以3D世界公尺換算，不再先畫固定圖示再二次縮放。
    const body=document.createElementNS('http://www.w3.org/2000/svg','rect');
    body.setAttribute('x',String(-bodyWidth/2));
    body.setAttribute('y',String(-bodyDepth/2));
    body.setAttribute('width',String(bodyWidth));
    body.setAttribute('height',String(bodyDepth));
    body.setAttribute('rx',String(Math.max(1,Math.min(bodyWidth,bodyDepth)*.12)));
    body.setAttribute('fill','#e89200');
    body.setAttribute('stroke','#222');
    body.setAttribute('stroke-width','1');
    body.setAttribute('class','outline plan-barrier-body');
    ...
    planPivot.setAttribute('cx',String(pivotX));
    planPivot.setAttribute('cy','0');
    planPivot.setAttribute('r',String(Math.max(1.5,bodyDepth*.16)));
    planPivot.setAttribute('fill','#15191c');
    planPivot.setAttribute('stroke','#7f8a90');
    planPivot.setAttribute('stroke-width','1');
    ...
    armGroup.setAttribute('transform',`translate(${armStartX} 0)`);

    const boomThickness=Math.max(2,.08*metrics.ppm.average);
    const arm=document.createElementNS('http://www.w3.org/2000/svg','rect');
    arm.setAttribute('x',String(sign>0?0:-armLength));
    arm.setAttribute('y',String(-boomThickness/2));
    arm.setAttribute('width',String(armLength));
    arm.setAttribute('height',String(boomThickness));
    arm.setAttribute('rx',String(Math.max(.8,boomThickness*.18)));
    arm.setAttribute('class','plan-barrier-boom outline');
    armGroup.appendChild(arm);

    const reflectorWidth=Math.max(3,.28*metrics.ppm.average);
    const reflectorGap=Math.max(7,.72*metrics.ppm.average);
    const reflectorStart=Math.max(4,.35*metrics.ppm.average);

    for(
      let distance=reflectorStart;
      distance+2<armLength;
      distance+=reflectorGap
    ){
      const visibleWidth=Math.min(
        reflectorWidth,
        Math.max(0,armLength-distance-2)
      );
      ...
      stripe.setAttribute('y',String(-boomThickness*.34));
      stripe.setAttribute('width',String(visibleWidth));
      stripe.setAttribute('height',String(boomThickness*.68));
    ...
    model.dataset.worldScaled='true';
    model.dataset.planWidth=String(bodyWidth+armLength);
    model.dataset.planHeight=String(Math.max(bodyDepth,boomThickness));
    model.dataset.worldWidth=String(.34+(Number(p.armLength)||4));
    model.dataset.worldDepth='.28';
    ...
    model.appendChild(svgElement('rect',{x:-width*getPlanPixelsPerMeter().x/2+3,y:-length*getPlanPixelsPerMeter().z/2+3,width:Math.max(1,width*getPlanPixelsPerMeter().x-6),height:Math.max(1,length*getPlanPixelsPerMeter().z-6),rx:7,fill:'none',stroke:'#1d2428','stroke-width':2}));
    model.appendChild(svgElement('rect',{x:width*getPlanPixelsPerMeter().x/2-5,y:length*getPlanPixelsPerMeter().z/2-7,width:15,height:9,rx:2,fill:'#d6aa27',stroke:'#735b12'}));
    model.dataset.worldScaled='true';
    model.dataset.trueTopView='true';
    model.dataset.planWidth=String(width*getPlanPixelsPerMeter().x);
    model.dataset.planHeight=String(length*getPlanPixelsPerMeter().z);
    model.dataset.worldWidth=String(width);
    model.dataset.worldDepth=String(length);
  ...
  // outer負責位置與標籤，model只負責設備外觀。
  // 因此縮放設備時，名稱與高度文字不會一起變形。
  // V4.3.8：2D改為真正俯視輪廓。
  // 既有正面SVG僅作為舊版相容來源，不再拿來決定尺寸比例。
  const trueTopViewResult=rebuildPlanModelAsTrueTopView(
   model,
   i,
   planRotation
  );

  const syncedPlanModel=trueTopViewResult||(
   model.dataset.worldScaled==='true'
    ?{
  ...
  if(syncedPlanModel){
   ...
   outer.insertBefore(hitbox,outer.firstChild);
   ...
   if(i.id===state.selectedId){
    ...
    const selectionWidth=Math.max(12,syncedPlanModel.renderedWidth+8);
    const selectionHeight=Math.max(12,syncedPlanModel.renderedHeight+8);
    ...
    selection.setAttribute('class','plan-physical-selection');
    outer.insertBefore(selection,model);
   }
  }
 }
...
/* V4.3.8 Collapsible Sidebar Inspector */
(function(){
...
/* V4.3.8 Unlimited Loop Dimensions */
(function(){
...
/* V4.3.8 Realistic Rendering Foundation */
(async function(){
...
/* =========================================================
   V4.3.8 Reliable Scene Brightness
   ========================================================= */
(function(){
...
/* =========================================================
   V4.3.8 Mobile Workspace Helpers
   ========================================================= */
...
/* =========================================================
   V4.3.8 Professional 2D Selection Engine
   ========================================================= */
(function(){
...
/* V4.3.8: map unlabeled SVG groups to the nearest device label/item. */
(function(){
...
/* =========================================================
   V4.3.8 Stable Scene Plugin Host API
   ========================================================= */
...
/* =========================================================
   V4.3.8 Monte Carlo Scene UI Bridge
   ========================================================= */
(function(){
